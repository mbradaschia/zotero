...
- The GitHub MCP can only write to `mbradaschia/zotero`.
- **Do NOT use slash-separated branch names** (e.g. `plugin/reading-status`).
  In this environment, `create_branch` + `push_files` return success for
  such names but the branch never actually persists. Use a flat name like
  `claude-plugin-reading-status`. Always verify by listing branches and
  fetching a known file after pushing.
- Binary files (`.xpi`) are not safely pushable via `push_files` (string
  content). Commit them as base64 text (e.g. `dist/foo.xpi.b64`) and have
  the user decode locally with `base64 -d`.
...
- Don't change the plugin ID without coordinating: it is the identity Zotero
  ...
  on uninstall.
- Don't use slash-separated branch names when pushing via this MCP; they
  silently fail to persist. See "Pushing to GitHub from this environment".
